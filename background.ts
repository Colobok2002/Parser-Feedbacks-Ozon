...
const checkAndProcessFeedbacks = async () => {
  const { work, timer } = await new Promise<StorageResult>((resolve) => {
    chrome.storage.local.get(["work", "timer"], (result) => resolve(result as StorageResult));
  });

  console.log(work)
  if (work) {
    let newTimer = timer > 1 ? timer - 1 : 60;
    ...
    chrome.storage.local.set({ timer: 60 });
  }
};
...
chrome.runtime.onInstalled.addListener(() => {
  chrome.storage.local.set({ timer: 60, work: false });
});
